...
void Dictionary::add(const std::string& w) {
  ...
  if (word2int_[h] == -1) {
    entry e;
    e.word = w;
    // We want to include only the MeSH descriptors.
    // Set it to minCount to bypass the minCount condition
    if ((w[0] & 0xFF) == 0xCE && (w[1] & 0xFF) == 0xB5 &&
        (w[7] & 0xFF) == 0x64) {
...
void Dictionary::computeSubwords(
    ...
    std::vector<int32_t>& ngrams,
    std::vector<std::string>* substrings) const {
    /* This should be where it determines whether to find ngrams of regular
     * words or pass entity codes
     */
  for (size_t i = 0; i < word.size(); i++) {
    /*
    // for entities, pass (compare to \u200b)
    if ((word[i] & 0xFF) == 0xE2 && (word[i+1] & 0xFF) == 0x80 
            && (word[i+2] & 0xFF) == 0x8B) {
        break;
    }
    */
    std::string ngram;
    if ((word[i] & 0xC0) == 0x80) { // first 2 bits indicates continuation byte
      continue;
    }
    // i: iterate in byte, 
    // j: from the begining of a unicode start to the end of word
    // n: ngram characters
    for (size_t j = i, n = 1; j < word.size() && n <= args_->maxn; n++) {
      ...
      // until ngram condition is valid
      if (n >= args_->minn && !(n == 1 && (i == 0 || j == word.size()))) {
        // get the hash id of the ngram
...
void Dictionary::initNgrams() {
  for (size_t i = 0; i < size_; i++) {
    ...
    bool isEntity = ((words_[i].word[0] & 0xFF) == 0xCE &&
                     (words_[i].word[1] & 0xFF) == 0xB5 &&
                     (words_[i].word[7] & 0xFF) == 0x64);
    if (!isEntity && words_[i].word != EOS) {
